...
This is particularly relevant in the context of this thesis' topological approach, since hypergraph representations can naturally embed logical relationships into a geometric form that we can apply TDA methods to. \newline \par \quad
2. An entailment relationship $\alpha \models \beta$, corresponds to a subsumption relationship between two hypergraphs on $\wp(U)$, such that $H_{\alpha} \sqsubseteq H_{\beta}$. The subsumption relationship is defined as true if: 
$$
\forall H, H' \in \mathcal{H},\; H \sqsubseteq H'
\iff 
\forall E \in H,\; \exists E' \in H' \text{ such that } 
\forall e \in E,\; \exists e' \in E' \text{ such that } e \leq e'
$$
...
- Why importan <-- relevant for topological approach because mebeds logical relationships into geometric structure...

Entailment corresponds to subsumption -- give example

Subsumption forms a De-Morgan Lattice structure <-- move onto lattice-theoretic properties... 
